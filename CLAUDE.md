...
- **2026-08-29 — Bridge v0.1 shipped** (`bridge/`, Laravel 13): typed read/write client over omniJS, audit-logged mutations, guarded cascade deletes, and an MCP server (`laravel/mcp`, stdio) exposing 16 tools. Registered for Claude Code via `.mcp.json` (restart the session to pick it up). Run tests: `cd bridge && ./vendor/bin/pest` (unit/feature) or `./vendor/bin/pest --group=integration` (live OmniFocus). Start server manually: `php bridge/artisan mcp:start omnifocus`. Next phase: GTD agent layer (inbox-triage skill, weekly-review coach), then streamable HTTP + Tailscale for Linux.
